 ...
 *       colorReminder: true,
 *       notify: true,
 *       notifyThreshold: "warn",   // "warn" | "over"
 *       locale: "zh",
 *     },
 *     migrations: [ "v1" ],
 *   }
  ...
  applyConfigPatch(patch) {
    ...
    for (const [key, val] of Object.entries(patch ?? {})) {
      if (key === 'quotas') {
        ...
        next.notify = val === true
      } else if (key === 'notifyThreshold') {
        if (val === 'warn' || val === 'over') next.notifyThreshold = val
        else errors.push('notifyThreshold: must be warn or over')
      } else if (key === 'locale') {
