...
function readModelsFromSettings(ctx) {
    ...
    if (llm && typeof llm.listProviders === 'function') {
      for (const p of llm.listProviders()) {
        if (p && typeof p.id === 'string' && p.id.length > 0) {
          ...
          providerInfoMap.set(p.id, { name: p.name || p.id })
  ...
  const addModels = (providerId, providerName, models) => {
  ...
  const handleSection = (ns, section) => {
    ...
    if (Array.isArray(section.models)) {
      ...
      const pid = routeOf(section.provider)
        ?? routeOf(ns.slice(4))
        ?? (section.provider || ns.slice(4))
      // provider 显示名:先用注册路由 info.name,再用 section.providerName,回退到 pid。
      let pname
      const info = providerInfoMap.get(pid)
      if (info && info.name) pname = info.name
      else if (section.providerName) pname = section.providerName
      else pname = pid
      addModels(pid, pname, section.models)
    }
    if (section.providers && typeof section.providers === 'object') {
      for (const [pid0, cfg] of Object.entries(section.providers)) {
        if (cfg && typeof cfg === 'object') {
          // 嵌套结构:dict key 通常就是路由(如 db),但以注册路由为准。
          const pid = routeOf(cfg.provider)
            ?? routeOf(pid0)
            ?? (cfg.provider || pid0)
          let pname
          const info = providerInfoMap.get(pid)
          if (info && info.name) pname = info.name
          else if (cfg.displayName || cfg.name) pname = cfg.displayName || cfg.name
          else pname = pid
          addModels(pid, pname, cfg.models)
        }
      }
    }
  }
  ...
  try {
    ...
    if (existsSync(settingsFile)) {
      const doc = loadYaml(readFileSync(settingsFile, 'utf-8'))
...
function buildState(ledger, ctx) {
  ...
  // 模型列表只以 settings 里配置的为准:删除模型后不会残留(历史账本里的模型不再显示在设置页)。
  // dock 里当前使用的模型若不在列表中,由客户端单独兜底。
