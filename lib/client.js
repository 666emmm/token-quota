  ...
  factory: (require) => {
    ...
    const MESSAGES = {
      zh: {
        ...
        notify: '系统通知(首次触达阈值时)',
        notifyThreshold: '触发条件',
        notifyWarn: '提醒阈值(80%)',
        notifyOver: '超额(100%)',
        dockChip: '{name} {used}/{quota} ({pct}%)',
        dockNoQuota: '{name} {used} (未设额度)',
        warnTitle: 'Token 额度提醒',
        warnBody: '{name} 今日已用 {pct}% 额度,建议切换模型',
        overTitle: 'Token 额度超额',
        overBody: '{name} 今日额度已用完({used}/{quota}),请切换模型',
      ...
      en: {
        ...
        notify: 'System notification (first hit only)',
        notifyThreshold: 'Trigger at',
        notifyWarn: 'Warn (80%)',
        notifyOver: 'Over (100%)',
        dockChip: '{name} {used}/{quota} ({pct}%)',
        dockNoQuota: '{name} {used} (no quota set)',
        warnTitle: 'Token Quota Warning',
        warnBody: '{name} has used {pct}% of today\'s quota — consider switching',
        overTitle: 'Token Quota Exceeded',
        overBody: '{name} quota exhausted ({used}/{quota}), please switch models',
    ...
    function SettingsSection(props) {
      ...
      const [draftNotify, setDraftNotify] = useState(snap?.config?.notify ?? false)
      const [draftThreshold, setDraftThreshold] = useState(snap?.config?.notifyThreshold ?? 'warn')
      ...
      useEffect(() => {
        ...
        setDraftNotify(snap?.config?.notify ?? false)
        setDraftThreshold(snap?.config?.notifyThreshold ?? 'warn')
      }, [snap?.config?.colorReminder, snap?.config?.notify, snap?.config?.notifyThreshold])
      ...
      const buildPatch = useCallback(() => {
        ...
        return { quotas, colorReminder: draftColor, notify: draftNotify, notifyThreshold: draftThreshold }
      }, [models, rows, perModel, draftColor, draftNotify, draftThreshold])
      ...
      return el('div', { className: 'tq-section' },
          ...
          : el('table', { className: 'tq-table' },
            el('thead', null,
              el('tr', null,
                ...
                el('th', null, t('colProvider')),
                el('th', { className: 'num' }, t('colUsed')),
                el('th', null, ''),
                el('th', { className: 'num' }, t('colQuota')),
            ...
            el('tbody', null,
              models.map(m => {
                ...
                return el('tr', { key: m.key },
                  ...
                  el('td', null, el('div', { className: 'tq-provider' }, m.provider)),
        ...
        el('div', { className: 'tq-row' },
          ...
          el('select', {
            ...
            disabled: !draftNotify,
            onChange: e => setDraftThreshold(e.target.value),
          },
            el('option', { value: 'warn' }, t('notifyWarn')),
            el('option', { value: 'over' }, t('notifyOver')),
          ),
        ),
    ...
    // 记录"本次会话已通知过"的模型 key,避免重复发通知。
    const notifiedKeys = { warn: new Set(), over: new Set() }
    ...
    async function apply(ctx) {
      ...
      function QuotaSideEffect(props) {
        ...
        useEffect(() => {
          ...
          function evaluate(key, name) {
            const info = quotaLevelFor(key, today, config)
            if (info.level === 'none') return
            if (info.level === 'over' && worstLevel !== 'over') {
              worstLevel = 'over'; worstInfo = info; worstKey = key; worstName = name
            } else if (info.level === 'warn' && worstLevel === 'ok') {
              worstLevel = 'warn'; worstInfo = info; worstKey = key; worstName = name
            }
          }

          if (current?.key) {
            const m = models.find(x => x.key === current.key)
            evaluate(current.key, m?.name ?? current.model)
          }
          // 遍历所有模型(确保超额模型即使不是当前模型也会显示全局提醒?——
          // 用户主要关心"正在用的模型",所以只在当前模型没配额度时再考虑其他)。
          if (worstLevel === 'ok' && !current?.key) {
            for (const m of models) evaluate(m.key, m.name)
          }

          // 2) 设置 body 属性驱动 CSS 变色。
          if (config.colorReminder !== false) {
            if (worstLevel === 'warn' || worstLevel === 'over') {
              document.body.setAttribute('data-tq-level', worstLevel)
            } else {
          ...
          // 3) 系统通知:同一 (key, level) 在本页面生命周期内只发一次。
          if (config.notify && worstInfo && worstKey && worstName) {
            const threshold = config.notifyThreshold ?? 'warn'
            const shouldNotify =
              (threshold === 'over' && worstLevel === 'over') ||
              (threshold === 'warn' && (worstLevel === 'warn' || worstLevel === 'over'))
            if (shouldNotify) {
              const set = notifiedKeys[worstLevel]
              if (!set.has(worstKey)) {
                set.add(worstKey)
                const t = makeT(resolveLocale(state))
                const title = worstLevel === 'over' ? t('overTitle') : t('warnTitle')
                const body = (worstLevel === 'over' ? t('overBody') : t('warnBody'))
                  .replace('{name}', worstName)
                  .replace('{pct}', Math.floor(worstInfo.pct * 100))
                  .replace('{used}', formatTokens(worstInfo.used))
                  .replace('{quota}', formatTokens(worstInfo.quota))
                try {
